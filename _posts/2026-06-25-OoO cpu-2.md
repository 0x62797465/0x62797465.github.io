...
As you can see, this is directly setting registers which affect the memory array that the AGU uses. Since we have to avoid multiple drivers, this logic is embedded inside the AGU. The word is also forwarded outside of the module:
...
This looks somewhat wonky because of the non-blocking assignments, but essentially it splits word writes into two buffers while keeping the writes in bounds. If you can recall, the two buffers exist to increase our bandwidth since we read 8 bytes at a time in order to achieve a 2-instruction-wide CPU. 
...
- We only need to set a single bit high for one cycle to indicate readiness and fill a 1-byte register in order to transmit a byte
...
As you may notice, a read has to also write to a status bit. To make things more complicated, the writer has to be speculative. Additionally, the receiver can only overwrite the byte if the previous byte was architecturally read, which requires setting the architectural and speculative status bit. For writing, it is slightly simpler. We need to speculatively write to the status register and transmission buffer, then copy it over into the real thing during the commit stage.
...
for (i = 0; i < 2; i = i + 1) begin
	if (update_bht[i]) begin
		if ((taken[i] == 2'b11 && bht[bht_ent_q[bht_head]] != 2'b00)
			|| (taken[i] == 2'b01 && bht[bht_ent_q[bht_head]] != 2'b11))
				bht[bht_ent_q[bht_head]] <= bht[bht_ent_q[bht_head]] + taken[i]; // "taken" will be 1 or negative 1
			bht_head = bht_head + 1;
	end
end
bht_head <= '0;
...
B_type : begin // assigns src1, src2, and immediate
		bht_ent_q[bht_tail] <= prev_fetch_addr + (i*4);
		bht_tail = bht_tail + 1;
		if (bht[bht_addr[5:0]][1]) begin
...
## Donut!
To load the donut program onto our FPGA, we must wrap the `printf`/`puts`/`putchar` calls in UART helper functions, add helpers for multiplication and division, compile it with our specific instruction set (bare RV32I) and a small assembly helper that sets the stack pointer, then copy over the ELF into a raw machine code file. To transmit, we have to configure our UART to the right baud rate and make it raw, then send the header, then the raw binary, and then read the output:
...
First things first, we have to make the multiplication unsigned. This is because two's complement does not apply to multiplication (except in the case where we take the lower 32 bits). To do this, we use combinational logic (which allows us to feed it to the DSP faster), which is dependent on the opcode passed through.
...
`OP` handling is relatively similar, although we have to store our signs in registers because the next instruction can overwrite any combinational logic (because there is a one-cycle delay between setting the ready signal and issue receiving it, so issue may fill the `uop` data). 
...
## Conclusion
CPUs are complex; they execute instructions before knowing they should via speculation, optimize speculation by predicting what code will execute, differentiate between an architectural state and a speculative state, and the list goes on. By contrast, before learning anything about CPU microarchitecture, I thought they were a large switch statement that just executed whatever instruction was fed into it, as my only exposure to CPUs was cheap emulators found in reverse-engineering CTF challenges. But, after this project's completion, I understand at least a portion of the ridiculous complexity of modern CPU architecture. Additionally, running this CPU on an FPGA taught me the importance of testbenches and copious amounts of code review on systems that are extremely hard to debug. Overall, despite the limited scope, this project has taught me a lot, and I hope the resulting write-up has taught you something as well!
