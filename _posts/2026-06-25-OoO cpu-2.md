...
[demo donut with emulated mul/div](https://github.com/user-attachments/assets/982bf816-b71f-41e1-858d-8100d113423e)

## RV32IM
The demo is pretty slow. Possibly a frame every two seconds. Luckily, the reason why is very clear: we are emulating multiplication and division at a very heavy penalty, and donuts needs a lot of multiplication and division. 

### Multiplication
Multiplication is pretty easy, since shifts represent multiplication by a power of 2, we can decay a single multiplication into multiple additions of multiplications by the power of 2. Luckily, we do not have to implement this by hand, and we can use DSPs to make it have a single cycle delay. 
...
### Division
Division is a bit more heavy. We compute the quotient by determining one bit at a time, starting from the highest bit. This is not as parallel multiplication, which is why we need many more cycles to do this. Additionally, it is variable cycle because our remainder can become zero before we compute the entire quotient. 

`OP` handeling is relatively similar, although we have to store our signs in registers because the next instruction can overwrite any combinational logic (as there is a one cycle delay between the ready signal, so issue may fill the `uop` data). 
...
The implementation of exit-handling is copying over output values based off `op` types, so I will omit it. 

## Donut!

[demo donut with regular mul/div](https://github.com/user-attachments/assets/8c0d74dd-aab1-47e5-9b85-e926fd8c09b4)
